...
**What to look for.**
- Loss around 0.15–0.20 and ~84% accuracy within 4000 iterations (about 20 s); psychometric curves that depend on the *cued* coherence and are flat in the uncued one. If they are not flat, the network is not doing the task, and nothing downstream means anything. Before anything else, finite-difference-check `grads` against `L` — it agrees to six digits, and if yours does not, stop.
- Run `slow_points` with `u0 = np.array([0,0,1,0])` and `[0,0,0,1]`. The returned $x$'s have median $q\approx10^{-3}$ and lie on an approximately one-dimensional manifold rather than at isolated points. That manifold is the (approximate) line attractor.
- At each slow point compute `J = -np.eye(N) + W*(1-np.tanh(x)**2)` and its eigenvalues. You will find a leading eigenvalue around $-0.15$, with the rest further into the left half plane. Note what that is: **not** an exact line attractor but a leaky integrator with a memory time constant of a few times the trial length divided by $T$. This is the honest and instructive result — gradient descent tunes only as precisely as the task demands, and the task here needs 15 time constants of memory, not infinity. Exercise 2 quantifies the tolerance; lengthen the trial and watch $|\lambda_1|$ shrink.
- Now the payoff: compute the right eigenvector $r_1$ and left eigenvector $l_1$ of the slow mode in each context, and evaluate $|l_1^\top B|$ on the two evidence channels. With the seed above you should get gains of roughly $(5.4,\,1.6)$ in context A and $(0.5,\,3.7)$ in context B — a three- to eight-fold selection of the cued channel — while $\cos(r_1^A, r_1^B)\approx0.7$: the integration axis is largely shared, the input gain is not. That is Mante et al.'s mechanism, reproduced in sixty lines. Report the numbers across seeds rather than trusting one, per §5.
